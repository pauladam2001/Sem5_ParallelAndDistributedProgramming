public class Algorithms {
    ...
    public static Polynomial multiplySequence(Polynomial p1, Polynomial p2, int start, int end) {
        // we go from coefficient on position start, until coefficient on position end - 1 (on p1), and multiply it with all the coefficients from p2
    ...
    public static Polynomial ClassicSequential(Polynomial p1, Polynomial p2){
    ...
    public static Polynomial KaratsubaSequential(Polynomial p1, Polynomial p2){
        if (p1.getDegree() < 2 || p2.getDegree() < 2)
            return ClassicSequential(p1, p2);
        ...
        Polynomial res1 = Polynomial.addZeroes(z2, 2 * m);
        Polynomial res2 = Polynomial.addZeroes(Polynomial.subtract(Polynomial.subtract(z1, z2), z0), m);

        return Polynomial.add(Polynomial.add(res1, res2), z0);
    }
}
